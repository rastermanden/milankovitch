...
const PANELS = [
  ...
  { key: 'dT', title: 'Surface temperature vs today (Stefan–Boltzmann, Planck only)', color: '#ffd98a', min: -1.6, max: 0.8, fmt: (v) => (v >= 0 ? '+' : '−') + Math.abs(v).toFixed(2) + ' K', zero: true },
];
...
export class TimeChart {
  ...
  draw() {
    ...
    PANELS.forEach((p, i) => {
      ...
      ctx.font = `10px ${font}`;
      ctx.textAlign = 'right';
      ctx.fillText(p.fmt(p.max).replace(' W/m²', '').replace(' K', ''), x0 - 6, y0 + 9);
      ctx.fillText(p.fmt(p.min).replace(' W/m²', '').replace(' K', ''), x0 - 6, y1);
